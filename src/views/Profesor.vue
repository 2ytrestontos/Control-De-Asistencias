<template>
  <div v-if="
    ...
    <div class="form-group row">
      ...
      <div class="col-sm-10">
        <input v-model="input.usuario" type="text" class="form-control" id="inputEmail3" placeholder="Nombre" />
      </div>
    </div>
    <div class="form-group row">
      <!--<label for="inputPassword3" class="col-sm-2 col-form-label">Contraseña</label-->
      <div class="col-sm-10">
        <input v-model="input.pass" type="password" class="form-control" id="inputPassword3" placeholder="Contraseña" />
      </div>
    </div>
    <div v-if="nombre == $store.state.sesion">
      <button disabled class="btn btn-danger" v-on:click="borrar(input.usuario)">
        Eliminar Profesor
      </button>
      <button class="btn btn-primary" v-on:click="actualizar($route.params.id, input.usuario, input.pass)">
        Actualizar Profesor
      </button>
    </div>
    <div v-else>
